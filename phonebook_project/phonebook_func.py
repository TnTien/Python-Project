...
def addToList(self):
    ...
    var_fname = var_fname.title()   #   This wil ensure that the first character in each word is capitalized
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0):   #   ensure that there is data present
        ...
        with conn:
            ...
            if count == 0 or count2 == 0:   #   if proposed changes are not already in the database, then proceed
                ...
                messagebox.showinfo("No changes detected","Both ({}) and ({}) \nalready exist in the database for this name. \n\nYour update reqeust has been cancelled.".format(var_phone, var_email))
            onClear(self)
        conn.close()
    else:
